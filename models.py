...
class SiameseGenerator(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(SiameseGenerator, self).__init__()

        self.c0 = nn.Conv2d(in_channels, 32, 4, stride=2, padding=1)
        self.c0_ = nn.Conv2d(in_channels, 32, 4, stride=2, padding=1)
        self.c1 = nn.Conv2d(32, 64, 4, stride=2, padding=1)
        self.c1_ = nn.Conv2d(32, 64, 4, stride=2, padding=1)
        self.c2 = nn.Conv2d(64, 128, 4, stride=2, padding=1)
        self.c2_ = nn.Conv2d(64, 128, 4, stride=2, padding=1)
        self.c3 = nn.Conv2d(128, 256, 4, stride=2, padding=1)
        self.c3_ = nn.Conv2d(128, 256, 4, stride=2, padding=1)
        self.c4 = nn.Conv2d(256, 256, 4, stride=2, padding=1)
        self.c4_ = nn.Conv2d(256, 256, 4, stride=2, padding=1)
        self.c5 = nn.Conv2d(256*2, 256, 4, stride=2, padding=1)
        self.c6 = nn.Conv2d(256, 256, 4, stride=2, padding=1)
        self.c7 = nn.Conv2d(256, 256, 4, stride=2, padding=1)

        self.d7 = nn.ConvTranspose2d(256, 256, 4, stride=2, padding=1)
        self.d6 = nn.ConvTranspose2d(256*2, 256, 4, stride=2, padding=1)
        self.d5 = nn.ConvTranspose2d(256*2, 256, 4, stride=2, padding=1)
        self.d4 = nn.ConvTranspose2d(256*2, 256, 4, stride=2, padding=1)
        self.d3 = nn.ConvTranspose2d(256*2, 128, 4, stride=2, padding=1)
        self.d2 = nn.ConvTranspose2d(128*2, 64, 4, stride=2, padding=1)
        self.d1 = nn.ConvTranspose2d(64*2, 32, 4, stride=2, padding=1)
        self.d0 = nn.ConvTranspose2d(32*2, out_channels, 4, stride=2, padding=1)
        ...
        self.bnc1_ = nn.BatchNorm2d(64)
        self.bnc2_ = nn.BatchNorm2d(128)
        self.bnc3_ = nn.BatchNorm2d(256)
        self.bnc4_ = nn.BatchNorm2d(256)
    ...
    def forward(self, x, x_):
        ...
        en4 = self.bnc4(self.c4(F.leaky_relu(en3, negative_slope=0.2)))
        en0_ = self.c0_(x_)
        en1_ = self.bnc1_(self.c1_(F.leaky_relu(en0_, negative_slope=0.2)))
        en2_ = self.bnc2_(self.c2_(F.leaky_relu(en1_, negative_slope=0.2)))
        en3_ = self.bnc3_(self.c3_(F.leaky_relu(en2_, negative_slope=0.2)))
        en4_ = self.bnc4_(self.c4_(F.leaky_relu(en3_, negative_slope=0.2)))
        en4_concate = torch.cat((en4, en4_), 1)
        en5 = self.bnc5(self.c5(F.leaky_relu(en4_concate, negative_slope=0.2)))
        en6 = self.bnc6(self.c6(F.leaky_relu(en5, negative_slope=0.2)))
        en7 = self.c7(F.leaky_relu(en6, negative_slope=0.2))
